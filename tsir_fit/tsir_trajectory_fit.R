...
if (fe <- file.exists("tsir_trajectory_fit.rda")) {
	load("tsir_trajectory_fit.rda")
}
...
for (i in (init+1):nsim) {
	...
	start <- c(log.beta=0.44, logit.rprob=qlogis(0.7), logit.I0=qlogis(10/1e5), log.size=log(1))
	...
	cdata <- data.frame(
		...
		lwr=c(exp(cc[1,1]), plogis(cc[2,1]), NA, NA),
		upr=c(exp(cc[1,2]), plogis(cc[2,2]), NA, NA)
	)
	...
	save("fitlist", file="tsir_trajectory_fit.rda")
}
