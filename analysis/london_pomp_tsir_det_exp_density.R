...
source("../R/fitfun_pomp_tsir_exp_det.R")
source("../R/basis.R")
...
## density dependence is equivalent to assuming fixed population size 
ld_covar$pop <- mean(ld_covar$pop) 
...
london_pomp_tsir_det_exp_density_list <- vector('list', 100)
...
for (i in 1:100) {
	...
	london_pomp_tsir_det_exp_density_list[[i]] <- list(mle=m, ll=ll)
}

save("trans.param", "london_pomp_tsir_det_exp_density_list", "ld_pomp_arg", 
	 file="london_pomp_tsir_det_exp_density.rda")
