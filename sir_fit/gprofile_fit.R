...
objfun <- function(param, 
	...
	ind <- seq(1, nrow(data)*10+1, by=10)
	
	nll <- -sum(dnbinom(data$incidence, mu=diff(out$CI[ind])*rprob, size=size, log=TRUE)) +
		sum((log(transfun(times)*rprob) - log(beta * out$S * exp(out$logI)/N*rprob))^2)
...
save("fitlist", file="gprofile_fit.rda")
